			...
			for(my $a=0;$a<=scalar(@Ref_genome_start);$a++)
				{
		 		if ($Q_genome_miss_start[$a] != "")
					...
					if ($CDS_start[$cds] >= $Q_genome_miss_start[$a] && $CDS_end[$cds] <= $Q_genome_miss_end[$a])
						...
						if ($cds == 0)
							{
							complete_mr($cmr_len, $CDS_end[$cds], $cds, $a, $pdt, $protein_id, $note);
							}
...
sub complete_mr
	{
	my ($cmr_len, $cmr_tmp, $cds, $a, $pdt, $protein_id, $note) = @_;
...
sub partial_mr
        {
        my ($pmr_len, $pmr_tmp, $cds, $a, $trim_cds_percent, $pdt, $protein_id, $note) = @_;
